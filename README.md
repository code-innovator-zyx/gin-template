...
```
用户(User) → 角色(Role) → 资源(Resource)  [实际授权路径]
    ↓           ↓              ↓
 Alice      Admin       GET /api/v1/users
  Bob       Editor      POST /api/v1/posts
                           ↓
                    权限组(Permission) [仅UI分组]
```

**新架构特点：**
- 🎯 **直接授权** - 角色直接绑定资源，权限验证更快
- 🎨 **UI友好** - 权限组用于前端展示分组
- 🔐 **安全优先** - 默认拒绝，明确授权
- ⚡ **高性能** - 多级缓存，权限检查仅需2ms
...
**架构说明：**
- **实际授权**：User → Role → Resource (通过 `role_resources` 表)
- **UI展示**：Resource 可选关联 Permission (通过 `permission_id` 字段)
...
-- 资源表（实际的权限控制对象）
resources (id, path, method, description, permission_id, created_at, updated_at)
          -- permission_id 可选，仅用于UI分组

-- 用户-角色关联表
user_roles (id, user_id, role_id)

-- 角色-资源关联表（实际授权）
role_resources (id, role_id, resource_id)
```

**关键变化：**
- ❌ 删除 `role_permissions` 表
- ✅ 新增 `role_resources` 表（实际授权）
- 📝 `permissions` 表仅用于UI分组展示
...
// 4. 查询数据库（新架构：直接查询 role_resources）
...
- ✅ **灵活性** - 支持一用户多角色
- ✅ **安全性** - 默认拒绝策略
- ✅ **性能优化** - 查询路径更短，减少JOIN操作
- ✅ **多种缓存** - 支持Redis/LevelDB/Memory三种缓存
- ✅ **可维护性** - 清晰的双层模型（授权层+展示层）
- ✅ **可扩展性** - 易于添加新资源和权限
- 🎨 **UI友好** - Permission用于前端展示分组
- 🎯 **细粒度** - 直接控制到每个API资源
...
<details>
<summary><b>3. 如何配置权限？（新架构）</b></summary>

1. 创建权限组（可选，仅用于UI分组）：POST /api/v1/permissions
2. 创建角色：POST /api/v1/roles
3. 绑定资源到角色（实际授权）：POST /api/v1/roles/:id/resources
4. 分配角色给用户：POST /api/v1/users/:id/roles

**注意**：实际授权是通过绑定资源（Resource）到角色实现的，Permission仅用于UI展示分组。
