...
**双层设计：权限组（UI展示） + 资源（实际授权）**

### 权限组（Permission） - 仅UI分组
用于前端展示和管理界面的逻辑分组，不参与实际权限验证。

### 资源（Resource） - 实际授权
每个 API 路由都是一个资源（path + method），角色通过绑定资源获得实际访问权限。

系统启动时会：
1. 扫描所有 `SetPermission()` 声明
2. 自动提取唯一的权限组（仅用于UI分组）
3. 同步所有路由资源到数据库
4. 设置资源的 `permission_id` 用于UI分组展示
5. **自动将所有资源绑定给超级管理员角色**（实际授权）
...
系统启动时自动初始化
    ↓
创建权限组（UI分组） → 同步资源 → 设置资源分组
    ↓
创建超管角色 → 绑定所有资源到超管（实际授权）
    ↓
创建管理员账号 → 分配超管角色
    ↓
...
**关键理解：**
```
User → Role → Resource （实际权限验证路径）
              ↓
         Permission （仅用于UI展示分组）
```
...
系统启动时会自动：
- ✅ 发现 `custom:manage` 权限组（UI分组）
- ✅ 创建权限组到数据库
- ✅ 同步路由资源到数据库
- ✅ 设置资源的 permission_id（UI分组关联）
- ✅ 将资源绑定给超级管理员角色（实际授权）
...
[INFO]   - 同步路由资源到数据库...
[INFO]     ✓ 同步了 15 个路由资源
[INFO]   - 设置资源的权限组（UI分组关联）...
[INFO]     ✓ 成功为 15 个资源设置权限组
[INFO]   - 初始化超级管理员角色...
[INFO]     ✓ 创建角色: 超级管理员
[INFO]   - 绑定所有资源到超级管理员角色（实际授权）...
[INFO]     ✓ 成功绑定 15 个资源到超级管理员
[INFO]   - 初始化默认管理员用户...
[INFO]     ✓ 创建管理员用户: admin
[INFO]     ✓ 分配超级管理员角色
[INFO] ✓ RBAC 权限系统初始化成功
